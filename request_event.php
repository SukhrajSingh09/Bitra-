...
if (($_SESSION['role'] ?? 'user') === 'admin') {
    echo json_encode([
        "success" => false,
        "message" => "Admins can add events directly."
    ]);
    exit;
}
...
$stmt = $mysqli->prepare("
    INSERT INTO event_requests (
        user_id, title, society, building, room, event_date, event_time, description, type, status
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
");
...
if ($stmt->execute()) {
    echo json_encode([
        "success" => true,
        "message" => "Your event request has been sent."
    ]);
} else {
    echo json_encode([
        "success" => false,
